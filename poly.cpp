...
// https://codeforces.com/contest/1709/submission/165168347
...
int main() {
	...
	vector <Mint> cur(K + 1, Mint(1));
	for (int dep = n - 1; dep; dep--) {
		Poly<> nxt = Poly<>(cur) * Poly<>(cur);
		Mint presum(0);
		for (int i = nxt.deg(); i >= 0; i--) {
			presum += nxt.at(i);
			if (i <= K) {
				cur[i] = presum + nxt.at(i) * (K - i);
			}
		}
	}
	Poly<> ans = Poly<>(cur) * Poly<>(cur);
	printf("%lld\n", (LL)ans.at(f).x);
}
